...
from app_state_service import read_app_state_value
from position_service import PixelDetection, _should_use_ground_footprint, project_to_ground, take_position_result
from schemas import AssistantTurnRequest, CameraPositionConfig
...
def _message_response(message: EdgeAssistantMessage, actions: list[EdgeAssistantAction]) -> EdgeAssistantMessageResponse:
    ...
    pending_actions = [action for action in actions if action.state == "pending" and action.expires_at >= current]
    if any(action.action_type != "reply_choice" for action in pending_actions):
        next_input = "confirmation"
    elif pending_actions:
        input_candidates = [
            str((action.payload or {}).get("choice_label") or (action.payload or {}).get("reply") or "")[:48]
            for action in pending_actions
        ]
        input_candidates = [item for item in input_candidates if item]
...
def _assistant_water_gun_target(action: EdgeAssistantAction) -> tuple[float, float, str, str] | None:
    ...
    db = object_session(action)
    raw_config = read_app_state_value(db, "camera-position") if db is not None else {}
    config = CameraPositionConfig.model_validate(raw_config or {})
    fruit_detection = PixelDetection(
        candidate.label,
        candidate.confidence,
        float(candidate.bbox.get("x", 0)),
        float(candidate.bbox.get("y", 0)),
        float(candidate.bbox.get("width", 0)),
        float(candidate.bbox.get("height", 0)),
    )
    if config.calibrated and _should_use_ground_footprint(fruit_detection):
        bbox = candidate.bbox
        try:
...
def _s3_water_gun_offline_reason(db: Session, site_id: str, current: int) -> str | None:
    rows = list(db.query(EdgeDeviceRecord).filter(EdgeDeviceRecord.site_id == site_id).all())
    s3_rows = [
        row for row in rows
        if row.device_id == s3_device_id() or row.role == "sensor_actuator"
    ]
    if not s3_rows:
        # Keep compatibility with tests and deployments that have not reported
        # edge-device presence yet. Once the S3 has registered, safety checks are
        # based on its live presence row.
        return None
    row = max(s3_rows, key=lambda item: item.last_seen_at)
    if not row.online or row.last_seen_at < current - 30_000:
        return "普通 ESP32 当前离线，不能执行喷水；请检查供电、MQTT 或串口连接后重试。"
    return None
...
def decide_edge_assistant_action(db: Session, site_id: str, action_id: str, decision: str) -> EdgeAssistantActionResponse:
    ...
    if action.expires_at < current:
        ...
        offline_reason = _s3_water_gun_offline_reason(db, site_id, current)
        block_reason = offline_reason or c5_water_gun_block_reason(db, site_id)
        if block_reason:
            action.state = "failed"
            action.resolved_at = current
            action.payload = {
                **action.payload,
                "error": block_reason,
                "error_code": "device_offline" if offline_reason else "water_gun_unavailable",
            }
            db.commit()
        ...
        if target is None:
            ...
            action.payload = {
                **action.payload,
                "error": error_message,
                "error_code": "invalid_manual_target" if isinstance(action.payload.get("manual_target"), dict) else "position_expired",
            }
        else:
            ground_range_mm, bearing_deg, target_label, source = target

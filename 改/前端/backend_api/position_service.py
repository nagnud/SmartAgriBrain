...
FRUIT_SURFACE_HEIGHT_MM = 40.0
GROUND_FOOTPRINT_LABEL_KEYWORDS = (
    "apple",
    "fruit",
    "fruits",
    "banana",
...
GROUND_FOOTPRINT_LABEL_KEYWORDS = GROUND_FOOTPRINT_LABEL_KEYWORDS + (
    "\u82f9\u679c",
    "\u6c34\u679c",
    "\u679c\u5b50",
    "\u679c\u5b9e",
...
def _should_use_ground_footprint(detection: PixelDetection) -> bool:
    normalized_label = (detection.label or "").replace(" ", "").lower()
    return any(keyword in normalized_label for keyword in GROUND_FOOTPRINT_LABEL_KEYWORDS)
...
def project_to_ground(detection: PixelDetection, config: CameraPositionConfig) -> PositionCandidate | None:
    ...
    fruit_target = _should_use_ground_footprint(detection)
    if fruit_target:
        # Vision models often mark the visible fruit center, but the control
        # target for range/bearing is the point where the fruit touches the
        # floor. Use the bbox bottom center and project it on the ground plane.
        anchor_type = "footprint_center"
        anchor_x = detection.x + detection.width / 2
        anchor_y = detection.y + detection.height
    else:
    ...
    fruit_surface_anchor = fruit_target and anchor_type in {"visual_center", "surface_center"}
    ...
    height_is_valid = (
        not fruit_target
        and not fruit_surface_anchor
        and detection.height_estimate_provided
        and detection.height_confidence >= MIN_HEIGHT_CONFIDENCE
        and detection.estimated_height_mm < config.camera_height_mm
    )
    if fruit_surface_anchor and FRUIT_SURFACE_HEIGHT_MM < config.camera_height_mm:
        effective_height_mm = FRUIT_SURFACE_HEIGHT_MM
    else:
        effective_height_mm = detection.estimated_height_mm if height_is_valid else 0.0
    height_fallback = not height_is_valid and not fruit_surface_anchor

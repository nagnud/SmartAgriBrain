...
DYNAMIC_INTERVAL_MS = 100
DYNAMIC_TIMEOUT_MS = 3_000
MANUAL_FORWARD_MAX_MM = 1_200.0
MANUAL_HALF_WIDTH_MM = 1_200.0
WATER_GUN_DEVICE_MAX_RANGE_MM = 1_200.0
WATER_GUN_ACCEPTED_MAX_RANGE_MM = 5_000.0
...
class WaterGunTarget(BaseModel):
    # Vision may locate a target slightly beyond the water-gun's calibrated
    # reach. Accept it here, then clamp the device-visible command to 1200 mm.
    ground_range_mm: float = Field(ge=300, le=WATER_GUN_ACCEPTED_MAX_RANGE_MM)
...
def _clamp_device_range_mm(ground_range_mm: float) -> float:
    return min(WATER_GUN_DEVICE_MAX_RANGE_MM, max(300.0, float(ground_range_mm)))


def _manual_target_in_bounds(target: WaterGunTarget) -> bool:
    radians = math.radians(target.bearing_deg)
    device_range_mm = _clamp_device_range_mm(target.ground_range_mm)
    right = device_range_mm * math.sin(radians)
    forward = device_range_mm * math.cos(radians)
    return (
        forward >= -0.001
...
class WaterGunRuntime:
    ...
    def set_preview(self, site_id: str, request: WaterGunPreviewRequest) -> WaterGunStateResponse:
        ...
        with self._lock:
            ...
            state.mode = "static"
            state.ground_range_mm = _clamp_device_range_mm(request.ground_range_mm)
    ...
    def set_static(self, db: Session, site_id: str, request: WaterGunStaticRequest) -> WaterGunStateResponse:
        ...
        with self._lock:
            ...
            target_changed = (
                abs(state.ground_range_mm - _clamp_device_range_mm(request.ground_range_mm)) > 0.05

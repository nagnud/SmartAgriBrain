...
def sanitize_reply_choice(raw_payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    reply = safe_text(raw_payload.get("reply") or raw_payload.get("value"))
    if not reply:
        return None
    payload: Dict[str, Any] = {
        "reply": reply[:160],
        "choice_label": safe_text(raw_payload.get("choice_label") or raw_payload.get("label"), reply)[:48],
    }
    choice_group = safe_text(raw_payload.get("choice_group"))
    if choice_group:
        payload["choice_group"] = choice_group[:80]
    return payload
...
def sanitize_action(raw: Any, index: int) -> Optional[AssistantAction]:
    ...
    if action_type == "reply_choice":
        payload = sanitize_reply_choice(raw_payload)
    elif action_type == "navigate_view":
...
def build_assistant_prompt(payload: AssistantChatRequest, references: List[KnowledgeReference]) -> str:
    ...
    return (
        ...
        "\n允许的 action："
        "\n1 reply_choice: {\"reply\":\"点击后作为用户回答发送的文字\",\"choice_label\":\"按钮短标签\",\"choice_group\":\"同组选项标识\"}"
        "\n2 navigate_view: {\"view\":\"overview|realtime|history|disease|ai|control|knowledge|alarms\"}"
        "\n3 open_panel: {\"panel\":\"assistant|smart_control|knowledge|disease_upload\"}"
        "\n4 device_command: {\"command\":\"fan_on|fan_off|pump_on|pump_off|curtain_open|curtain_close|alarm_on|alarm_off\",\"value\":0或1,\"reason\":\"原因\"}"
        "\n5 smart_control: {\"operation\":\"enable|disable|all_auto|open_panel|set_manual|update_manual|restore_auto\",\"key\":\"water|light|heat|cool|vent|co2\",\"value\":0到100}"
        "\n6 knowledge_base: {\"operation\":\"create|update|delete|select\",\"kbId\":数字,\"name\":\"名称\",\"description\":\"描述\"}"
        "\n7 knowledge_item: {\"operation\":\"create|update|delete|select\",\"kbId\":数字,\"itemId\":数字,\"title\":\"标题\",\"content\":\"正文\"}"
        "\n8 run_knowledge_analysis: {\"kbId\":数字,\"question\":\"分析问题\"}"
        "\n9 refresh_data: {}"
        "\n页面没有独立补光灯开关；用户要求补光时只能调整智能托管的 light 参数，不能生成 light_on 或 light_off。"
        "未指定补光百分比时返回 30%、60%、90% 三个 smart_control update_manual actions 供用户点击选择。"
        "只要 answer 要求用户在两个或更多候选中选择，就必须为每个候选返回 reply_choice action，禁止只列文字选项后让用户手动输入。"

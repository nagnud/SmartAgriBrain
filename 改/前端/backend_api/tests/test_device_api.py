...
from assistant_orchestrator import (  # noqa: E402
    ...
    _web_reply_choice_actions,
    run_assistant_turn,
)
from schemas import AssistantTurnRequest, CameraPositionConfig, PositionCandidate  # noqa: E402
from position_service import PixelDetection, _remember, project_to_ground  # noqa: E402
...
class DeviceApiTests(unittest.TestCase):
    ...
    def test_command_queue_claim_ack_and_idempotency(self) -> None:
        ...
        self.assertIsInstance(queued.json()["message"], str)
        self.assertTrue(queued.json()["message"])
    ...
    def test_water_gun_static_dynamic_and_stop_protocol(self) -> None:
        self.assertEqual(DYNAMIC_INTERVAL_MS, 100)
        ...
        manual_outside = self.client.post(
            ...
            json={"ground_range_mm": 1701, "bearing_deg": 0, "source": "manual", "spray_enabled": True},
        )
        self.assertEqual(manual_outside.status_code, 200)
        self.assertEqual(manual_outside.json()["ground_range_mm"], 1200.0)
        self.assertEqual(manual_outside.json()["pump_control_percent"], 100.0)
        ...
        self.assertEqual(visual_outside.json()["ground_range_mm"], 1200.0)
    ...
    def test_water_gun_assistant_confirmation_blocks_when_s3_offline(self) -> None:
        candidate = PositionCandidate(
            id="offline-target",
            label="苹果",
            confidence=0.95,
        ...
        with SessionLocal() as db:
            db.add(EdgeDeviceRecord(
                ...
                status={"online": False, "reason": "unexpected_disconnect"},
            ...
            self.assertEqual(decided.state, "failed")
            self.assertEqual(decided.payload["error_code"], "device_offline")
            self.assertIn("普�?ESP32 当前离线", decided.payload["error"])
            self.assertEqual(db.query(DeviceCommandRecord).count(), 0)
    ...
    def test_water_gun_assistant_timed_confirmation_keeps_spraying_when_target_changes(self) -> None:
        ...
        self.assertEqual(decided.payload["water_gun"]["ground_range_mm"], refreshed.ground_range_mm)
    ...
    def test_water_gun_duration_follow_up_creates_timed_confirmation(self) -> None:
        ...
        self.assertIsInstance(reply.answer, str)
    ...
    def test_web_camera_water_gun_asks_duration_before_confirmation(self) -> None:
        ...
        with patch("assistant_orchestrator.deepseek_api_key", return_value="test-key"), patch(
            ...
            self.assertTrue(all(action.type == "reply_choice" for action in first.actions))
            self.assertTrue(all(action.payload.get("choice_label") for action in first.actions))
            # duration choices are rendered as clickable reply actions
            ...
            self.assertEqual(second.actions[0].payload["spray_duration_seconds"], 5)
    ...
    def test_web_assistant_identifies_visible_fruit_types(self) -> None:
        ...
        self.assertIn("梨、苹", reply.message.content)
    ...
    def test_c5_parameter_follow_up_keeps_context_and_dispatches_exact_percentage(self) -> None:
        with SessionLocal() as db:
            ...
            self.assertEqual(first.input_candidates, ["百分"])
            self.assertEqual(first.actions, [])

            invalid = create_edge_assistant_reply(
                db, "greenhouse_001", "高一", first.session_id, "edge_text"
            )
            ...
            completed = create_edge_assistant_reply(
                db, "greenhouse_001", "百分之八", first.session_id, "edge_text"
            )
    ...
    def test_c5_invalid_or_canceled_parameter_never_generates_command(self) -> None:
        with SessionLocal() as db:
            first = create_edge_assistant_reply(db, "greenhouse_001", "调整补光", None, "edge_text")
            invalid = create_edge_assistant_reply(db, "greenhouse_001", "百分之二", first.session_id, "edge_text")
            self.assertEqual(invalid.next_input, "parameter")
            ...
            self.assertIn("已取", canceled.content)
            self.assertEqual(db.query(SiteCommandRecord).count(), 0)
    ...
    def test_c5_expired_parameter_request_is_explained_without_model_retry(self) -> None:
        with SessionLocal() as db:
            ...
            expired = create_edge_assistant_reply(db, "greenhouse_001", "百分之八", first.session_id, "edge_text")
            self.assertIn("设置已超", expired.content)
    ...
    def test_c5_respects_s3_capabilities_offline_state_and_confirmation_recheck(self) -> None:
        with SessionLocal() as db:
            ...
            self.assertIn("暂不支持控制补光", unsupported.content)
    ...
    def test_c5_water_gun_requires_declared_s3_capability(self) -> None:
        with SessionLocal() as db:
            ...
            db.commit()
            reply = create_edge_assistant_reply(
                db, "greenhouse_001", "给番茄浇", None, "edge_text"
            )
            self.assertEqual(reply.actions, [])
    ...
    def test_web_light_request_offers_smart_control_choices_and_applies_selected_value(self) -> None:
        ...
        with SessionLocal() as db, patch(
            ...
            self.assertEqual(first.actions[0].type, "device_command")
            self.assertEqual(first.actions[0].payload["command"], "light_on")
            self.assertEqual(first.actions[0].payload["value"], 100)
            self.assertTrue(first.actions[0].description)
            self.assertEqual(db.query(SiteCommandRecord).count(), 0)

            selected = decide_edge_assistant_action(db, "greenhouse_001", first.actions[0].id, "confirm")
            ...
            self.assertEqual([action.state for action in stored_actions], ["confirmed"])
            queued = db.query(SiteCommandRecord).one()
            self.assertEqual((queued.target, queued.value, queued.source, queued.state), ("grow_light", 100, "web_manual", "queued"))
    ...
    def test_web_explicit_light_percentage_creates_one_smart_control_action(self) -> None:
        with SessionLocal() as db:
            ...
            self.assertEqual(len(reply.actions), 1)
            self.assertEqual(reply.actions[0].type, "device_command")
            self.assertEqual(reply.actions[0].payload["command"], "light_on")
            self.assertEqual(reply.actions[0].payload["value"], 75)

    def test_web_open_light_wording_defaults_to_full_grow_light_command(self) -> None:
        with SessionLocal() as db, patch("assistant_orchestrator.call_deepseek_chat_message") as model_call:
            ...
            self.assertEqual(reply.actions[0].type, "device_command")
            self.assertEqual(reply.actions[0].payload["command"], "light_on")
            self.assertEqual(reply.actions[0].payload["value"], 100)
    ...
    def test_web_inline_water_gun_duration_choices_are_clickable(self) -> None:
        actions = _web_reply_choice_actions("已找到苹果，需要喷多久�?0秒�?0秒�?分钟、持续喷")
        self.assertEqual([action.type for action in actions], ["reply_choice"] * 4)
        self.assertEqual(
            [action.payload["choice_label"] for action in actions],
            ["10 ", "30 ", "1 分钟", "持续喷水"],
        )
    ...
    def test_web_coordinate_water_gun_voice_typo_asks_direction_instead_of_using_camera(self) -> None:
        with patch("assistant_orchestrator.call_deepseek_chat_message") as model_call, patch(
            ...
            self.assertTrue(all("10" in str(action.payload.get("reply", "")) for action in reply.actions))
    ...
    def test_c5_manual_water_gun_keeps_direction_and_duration_across_voice_turns(self) -> None:
        with SessionLocal() as db, patch("assistant_orchestrator.call_deepseek_chat_message") as model_call, patch(
            ...
            self.assertEqual(first.input_candidates, ["左侧", "右侧", "不偏"])
    ...
    def test_c5_manual_water_gun_asks_and_keeps_duration_without_camera(self) -> None:
        with SessionLocal() as db, patch("assistant_orchestrator.call_deepseek_chat_message") as model_call, patch(
            ...
            self.assertEqual(first.input_candidates, ["10", "30", "1分钟", "持续喷水"])
            self.assertIn("喷射时长", first.content)

            second = create_edge_assistant_reply(
                db, "greenhouse_001", "三十", first.session_id, "edge_text"
            )
            model_call.assert_not_called()
            fresh_snapshot.assert_not_called()
            self.assertEqual(second.next_input, "confirmation")
    ...
    def test_v2_assistant_keeps_history_and_hidden_tool_context(self) -> None:
        ...
        self.assertTrue(any(isinstance(item.get("content"), str) and item.get("content") for item in second_turn_messages))
    ...
    def test_camera_configuration_question_prefetches_saved_height(self) -> None:
            ...
            if model_calls == 1:
                ...
                return {
                    ...
                    "tool_calls": [{
                        ...
                        "type": "function",
                        "function": {"name": "locate_camera_target", "arguments": '{"query":"摄像头高"}'},
                    }],
                }
            return {
                "role": "assistant",
                "content": '{"answer":"镜头到定位平面的高度�?110 mm","actions":[],"referenceIds":[]}',
            }
        ...
        self.assertEqual(response.message.content, "已保存的镜头安装高度�?110 mm�?1 cm），指镜头到定位平面的垂直高度")

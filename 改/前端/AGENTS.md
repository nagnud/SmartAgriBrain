...
本文件适用于当前 `前端` 目录及其子目录中工作的所有代码 agent。

## 唯一源代码目录

- 当前 `前端/` 是 Web 前端、FastAPI 集成服务及配套脚本的唯一开发源。
- `.push-cache/` 是持久化 Git 推送队列，不是开发目录，不得手工清空、覆盖、重建或格式化。
- `SmartAgriBrain/` 是历史嵌套检出目录，不是开发源，不得把其中内容反向覆盖到当前目录。
- 推送脚本只把当前目录的受控快照镜像到远端仓库 `feature/frontend` 分支下的 `frontend_dashboard/`。脚本内部仍可能出现历史变量名 `pro`，它不表示另一个源码目录。
...
- 修改远端地址、目标分支、源目录或 `frontend_dashboard` 映射前，必须先获得用户明确同意，并同步更新本文件和项目根 `README.md`。

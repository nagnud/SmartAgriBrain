...
function Ensure-FrontendDependencies {
  ...
  $rolldownWinBinding = Join-Path $ProjectRoot "node_modules\@rolldown\binding-win32-x64-msvc"
  if ((Test-Path -LiteralPath $viteCli) -and (Test-Path -LiteralPath $rolldownWinBinding)) {
    return
  }
  ...
  try {
    & $NodeTools.Npm install --include=optional
    if ($LASTEXITCODE -ne 0) {

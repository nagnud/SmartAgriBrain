...
本文件第 1 至第 8 节仍保留 2026-07-11 的跨分支历史评审，不应被理解为当前代码快照。2026-07-21 至 2026-07-22 在 `feature/lcd-docs` 可直接核验的新进展如下：

- 普通 ESP32 工程已加入正式 `smartagribrain/v1` 主题、ESP-IDF MQTT QoS 1 持久会话和 TLS。
- 已实现 `pump`、`grow_light`、`pan`、`tilt` 原子命令，以及 `target_position` 水枪复合命令。
- 已实现 GPIO26 统一水泵出口、二维 SG90、静态/定时/动态水枪状态机、3 秒动态超时和 ACK 幂等缓存。
- 能力、遥测、状态和 ACK 已有设备端发布实现，PlatformIO `esp32dev` 构建通过。
- 新增 `前端/` 工作区已提供 FastAPI MQTT v1 接入、命令记录和水枪路由。后端与普通 ESP32 均使用 EMQX TLS；水枪控制仅走真实命令下发路径。
- Web heartbeat 已转换为递增序号的后端 MQTT 保活队列项；C5 仍未获得最终 ACK，且真机端到端验收尚未完成。
- ESP32-C5 源码不在当前工作区，语音、大模型和屏幕功能只能按职责设计描述，不能更新为已联调完成。

当前事实源依次为 [问题与答复汇总](open-questions.md)、[统一通信与数据标准](smartagribrain-v1-standard.md)、[ESP32 与后端通信要求](ESP32与后端通信.md) 和当前源码。本历史评审中的旧 C5 MQTT、旧执行器能力和旧主题示例不得用于新开发。

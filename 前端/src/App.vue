  ...
  try {
    if (action.payload._assistant_v2 === true) {
      ...
      if (!['confirmed', 'executed'].includes(decided.state)) {
        const backendError = typeof decided.payload.error === 'string' ? decided.payload.error.trim() : '';
        throw new UserFacingError(backendError || '后端没有接受这次确认，请重新发起操作。');
      }
...
async function selectAssistantReplyChoice(action: AssistantAction): Promise<void> {
  ...
  try {
    ...
    setAssistantActionStatus(action, 'failed', userErrorText(error, '选项没有提交，请重新选择。'));
    return;
  } finally {
    assistantExecutingActionId.value = null;
  }

...
async def analyze_disease_image(upload: UploadFile, retrieval_mode: RetrievalMode = "auto") -> Dict[str, Any]:
    ...
    vision_result = await run_in_threadpool(call_ark_vision, image_bytes, content_type)
    deepseek_result, references, retrieval_status = await run_in_threadpool(
        call_deepseek_vision_summary,
        vision_result,
        retrieval_mode,
    )

...
from crop_vision_service import detect_current_crop_positions, read_current_crop_positions, save_current_crop_positions
...
@app.post("/api/vision/crops/current", response_model=CurrentCropPositionsResponse)
async def post_current_crop_positions(
    image: UploadFile = File(...),
    calibration: str = Form(...),
    ...
    result = await run_in_threadpool(detect_current_crop_positions, image_bytes, content_type, config)
    return save_current_crop_positions(db, result)


@app.get("/api/vision/crops/current", response_model=CurrentCropPositionsResponse)
def get_current_crop_positions(db: Session = Depends(get_db)) -> CurrentCropPositionsResponse:
    return read_current_crop_positions(db)
    ...
    with SessionLocal() as db:
        ...
        if pending_action_id and pending_decision == "confirm":
            ...
            if action.state == "executed":
                answer = "操作已完成。"
            elif action.state == "confirmed":
                answer = "已确认，正在执行。"
            elif action.state == "expired":
                answer = "确认已超时，请重新发出请求。"
            else:
                error = str(action.payload.get("error") or "")
                answer = error if error else "暂时无法完成这项操作，请重新发出请求。"
...
def _voice_reply(
    *,
    ...
    # C5 answers can contain one short, non-repeating danger notice after the
    # user's main answer. Keep both semantic segments instead of cutting at 60.
    spoken_answer = voice_spoken_text(answer, limit=120)

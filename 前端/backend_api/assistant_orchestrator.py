...
_WEB_SELECTION_CUE = re.compile(
    ...
    r"请告诉我.{0,24}(?:哪个|哪一个|哪项|哪种|多少|档位)|确认还是取消"
)
...
def _fast_web_smart_light_reply(db: Session, text: str) -> _FastEdgeReply | None:
    ...
    if requested_value is not None:
        action = _web_light_smart_action(requested_value, current_value)
        return _FastEdgeReply(
            f"将把智能托管的补光参数从 {current_value}% 调到 {requested_value}%，请确认。",
            {"web_smart_light": True, "current_light_value": current_value},
            [action],
        )
    ...
    return _FastEdgeReply(
        f"页面没有独立补光灯开关。当前智能托管补光参数为 {current_value}%，请选择要调到的档位。",
        {"web_smart_light": True, "current_light_value": current_value},
        actions,
    )
...
def _web_light_device_action_as_smart_control(action: AssistantAction) -> AssistantAction:
    ...
    return action.model_copy(update={
        ...
        "description": f"调整智能托管的补光参数到 {requested_value}%，不会开启独立补光灯。",
        "risk": "medium",
        "payload": {
            "operation": "update_manual",
            "key": "light",
            "value": requested_value,
            "reason": str(action.payload.get("reason") or "网页助手调整智能托管补光参数"),
        },
    })

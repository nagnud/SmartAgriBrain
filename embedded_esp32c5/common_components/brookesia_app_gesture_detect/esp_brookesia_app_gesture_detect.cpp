...
void GestureDetect::gestureDetectEventThread()
{
    while (1) {
        ...
        if (bmi2_get_sensor_data(&sensor_data, bmi_handle_) == BMI2_OK) {
            /* Gesture mode configures the accelerometer for +/-16 g and gyro for +/-2000 dps. */
            sensair_sensor_update_imu(
                (float)sensor_data.acc.x / 2048.0f,
                (float)sensor_data.acc.y / 2048.0f,
                (float)sensor_data.acc.z / 2048.0f,
                (float)sensor_data.gyr.x / 16.4f,
                (float)sensor_data.gyr.y / 16.4f,
                (float)sensor_data.gyr.z / 16.4f);
        }
